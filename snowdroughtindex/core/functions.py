...
def hydrographs(basins, streamflow_obs, month_start_water_year, day_start_water_year):
    ...
    with warnings.catch_warnings():
        ...
        norm_nivalbasins = np.ones((len(basins.index),366))
        ...
        niv_med = np.quantile(norm_nivalbasins, [0,.1,.25,.5,.75,.9,1], axis=0)

        # plotting
        plt.plot(np.arange(366), niv_med[3,:], color='b', label='median')
        plt.fill_between(np.arange(366), niv_med[2,:], niv_med[4,:], color='b', alpha=.5, lw=0, label='25th to 75th perc. range')
        plt.fill_between(np.arange(366), niv_med[1,:], niv_med[2,:], color='b', alpha=.35, lw=0, label='10th to 90th perc. range')
        plt.fill_between(np.arange(366), niv_med[4,:], niv_med[5,:], color='b', alpha=.35, lw=0)
        plt.fill_between(np.arange(366), niv_med[0,:], niv_med[1,:], color='b', alpha=.15, lw=0, label='min. to max.')
        plt.fill_between(np.arange(366), niv_med[5,:], niv_med[6,:], color='b', alpha=.15, lw=0)
...
def polar_plot(theta_rad, regularity, flag, nival_start_doy, nival_end_doy, nival_regularity_threshold):
    ...
    # Plot circular statistics for multiple basins - i.e., individual events are not shown
        ...
        plt.plot(theta_rad, regularity, 'bo', ms=5)
